...
esp_err_t DShotRMT::init(bool wait) {
  ...
  ESP_LOGI(TAG, "Sending idle throttle");
  if (wait)
...
// Set telemetry bit on next packet.
esp_err_t DShotRMT::getTelemetry() {
  DShotRMT::telemetry = 0x01;
  return ESP_OK;
}
...
esp_err_t DShotRMT::writePacket(dshot_packet_t packet, bool wait) {
  ...
  data <<= 1;
  data |= DShotRMT::telemetry;
  //data |= packet.telemetry;
  data = (data << 4) | checksum(data);
  DShotRMT::telemetry = 0x00;
